...
public class DepositService : IDepositService
{
    ...
    public async Task DepositRequestAsync(Deposit deposit, string? description, int accountId,string? userId)
    {
        DepositRequestServices = new DepositRequest(_depositSet);
        await DepositRequestServices.DepositAsync(deposit, description, accountId, userId);
    }

    public async Task PreDepositRequestAsync(Deposit deposit, string? description, int accountId, string? userId)
    {
        DepositRequestServices = new PreDepositDepositRequest(_preDepositRequestServices, _depositSet, _unitOfWork);
        await DepositRequestServices.DepositAsync(deposit, description, accountId, userId);
    }

    public async Task WalletDepositRequestAsync(Deposit deposit, string? description, int accountId, string? userId)
    {
