using Stokvel_Groups_Home_RSA.Data;
using Stokvel_Groups_Home_RSA.Interface.IRepo.Finance;
using Stokvel_Groups_Home_RSA.Models;

namespace Stokvel_Groups_Home_RSA.Repositories
{
    public class WithdrawRepository : Repository<WithdrawDetails>, IWithdrawRepository
	{
		private readonly ApplicationDbContext _context;

		public WithdrawRepository(ApplicationDbContext context): base(context) 
		{
			_context = context;
		}


		public void Update(WithdrawDetails? withdrawDetails)
		{
			_context.WithdrawDetails.Update(withdrawDetails);
		}


	}
}
